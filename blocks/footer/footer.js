...
/**
 * Handles the first section separately
 */
...
/**
 * Handles middle sections (like footer lists) and generates the required navigation markup
 */
/**
 * Handles middle sections (like footer lists) and generates the required navigation markup
 */
function handleMiddleSections(child, block, iteration) {
  ...
  nav.classList.add('footer-navigation'); // Updated class
  nav.setAttribute('x-data', '{selectedList:null}');
  ...
  sections.forEach((section) => {
    ...
    if (list && list.tagName === 'UL') {
      ...
      sectionDiv.classList.add('footer-section'); // Updated class

      const titleDiv = document.createElement('div');
      titleDiv.classList.add('footer-section-title'); // Updated class
      titleDiv.textContent = sectionTitle;

      const ul = document.createElement('ul');
      ul.classList.add('footer-section-list'); // Updated class

      // Iterate through list items
      list.querySelectorAll('li a').forEach((link) => {
        const li = document.createElement('li');
        li.classList.add('footer-item'); // Updated class

        const a = document.createElement('a');
        a.classList.add('footer-link'); // Updated class
        a.href = link.href;
  ...
  // Select a single h2 element and append it after the nav
  const header = child.querySelector('h2');
  if (header) {
    const headerDiv = document.createElement('div');
    headerDiv.classList.add('footer-heading'); // Updated class
    headerDiv.textContent = header.textContent;
    wrapperDiv.appendChild(headerDiv);
    const hr = document.createElement('hr');
    hr.classList.add('footer-divider'); // Updated class
...
/**
 * Handles black sections in the footer
 */
...
/**
 * Adjusts the font size of the second wrapper's header
 */
function adjustHeaderFontSize(block) {
...
/**
 * Processes and appends the fragment sections to the footer block
 */
