...
// create filter wrapper to wrap category filter terms if selected filters are more than 1 only for mobile.
...
filterCountWrapper.addEventListener("click",function()
{
  ...
  const breadcrumbContainer=breadcrumb.querySelector(".facet-breadcrumb-container");
  ...
  const clearAllFiltersButton=breadcrumbContainer.querySelector("button");
  ...
  facetBreadcrumb.state.facetBreadcrumbs.forEach((value) => {
    value.values.forEach((item) => {
      ...
      if(canMobileActions())
        ...
        const clearAllFiltersBtn=breadcrumbContainer.getElementsByTagName("button");
